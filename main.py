...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='C:/Users/USER/Downloads/FTP/dataSet')
...
def main():
    ...
    fig, ax = plt.subplots(1, 2, figsize=(20, 10))
    fig_img_rocauc, fig_pixel_rocauc = ax[0], ax[1]
    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        test_dataset = mvtec.MVTecDataset(args.data_path, class_name=class_name, is_train=False)
        test_dataloader = DataLoader(test_dataset, batch_size=32, pin_memory=True, num_workers=4)
        ...
        if not os.path.exists(train_feature_filepath):
            ...
            for i in range(H * W):
                cov[:, :, i] = LedoitWolf().fit(embedding_vectors[:, :, i].T.numpy()).covariance_
        ...
        test_feature_filepath = os.path.join(args.save_path, f'temp_{args.arch}', f'test_{class_name}.pkl')
        if os.path.exists(test_feature_filepath):
        ...
        embedding_vectors = torch.index_select(embedding_vectors, 1, idx)
        B, C, H, W = embedding_vectors.size()
        embedding_vectors = embedding_vectors.view(B, C, H * W).permute(0, 2, 1).numpy()  # [B, H*W, C]

        dist_list = np.zeros((B, H * W))

        for i in range(H * W):
            mean_i = mean[:, i]
            inv_cov = np.linalg.inv(cov[:, :, i])
            delta = embedding_vectors[:, i, :] - mean_i  # shape: [B, C]
            dist_list[:, i] = np.sqrt(np.einsum('bi,ij,bj->b', delta, inv_cov, delta))
        ...
        fig_pixel_rocauc.plot(fpr, tpr, label=f'{class_name} pixel_ROCAUC: {per_pixel_rocauc:.3f}')
...
def embedding_concat(x, y):
    B, C1, H1, W1 = x.size()
    _, C2, H2, W2 = y.size()
    s = H1 // H2  # Downsample scale

    # Unfold x into patches
    x_unfold = F.unfold(x, kernel_size=s, stride=s)  # [B, C1*s*s, L] where L = H2*W2
    x_unfold = x_unfold.view(B, C1, s * s, H2, W2)

    # Expand y to match patch shape
    y_expand = y.unsqueeze(2).expand(-1, -1, s * s, -1, -1)  # [B, C2, s*s, H2, W2]

    # Concatenate along channel dim
    z = torch.cat((x_unfold, y_expand), dim=1)  # [B, C1+C2, s*s, H2, W2]
    z = z.view(B, -1, H2 * W2)
    z = F.fold(z, output_size=(H1, W1), kernel_size=s, stride=s)
